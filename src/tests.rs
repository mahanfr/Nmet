use crate::{compiler::{compile, OutputType}, utils::get_program_name, assemble_with_nasm, link_to_exc};
use std::{fs::remove_file, process::Command, path::Path};
...
macro_rules! test_asm {
    ($tname: ident, $in_path: expr, $res_path: expr) => {
        ...
        fn $tname() {
            ...
            let out_path = Path::new(&format!("./build/{program_name}")).to_owned();
            // Generate executable
            compile($in_path.to_string(), out_path.to_path_buf(), OutputType::Asm);
...
mod asm {
    ...
    test_asm!(binary_expr, "./tests/binary_expr.nmt","./tests/binary_expr.txt");
    test_asm!(cont_break,"./tests/cont_break.nmt", "./tests/cont_break.txt");
    test_asm!(compare_expr,"./tests/compare_expr.nmt","./tests/compare_expr.txt");
    test_asm!(string_expr,"./tests/string_expr.nmt", "./tests/string_expr.txt");
    test_asm!(structs,"./tests/structs.nmt", "./tests/structs.txt");
    test_asm!(loops,"./tests/loops.nmt", "./tests/loops.txt");
    test_asm!(conditions,"./tests/conditions.nmt", "./tests/conditions.txt");
